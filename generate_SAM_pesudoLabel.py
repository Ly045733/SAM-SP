...
parser.add_argument('--root_path', type=str,
                    default='/home/lxy/pycharm_project/model_New/Dataset/ACDC', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='ACDC/ACDC_iteration_vit', help='experiment_name')
parser.add_argument('--use_iter', type=int,
                    default=1, help='cross validation')
parser.add_argument('--model_type', type=str,
                    default='vit_h', help='cross validation')
...
if __name__ == '__main__':
    ...
    if model_type == 'vit_h':
        checkpoint = "/home/lxy/pycharm_project/model_New/SAM_model_path/sam_vit_h_4b8939.pth"
        save_best_path = '/home/lxy/pycharm_project/model_New/SAM_model_path/SAM_FineTune/ACDC/new/iter1/sam_best_decoder.pth'
    elif model_type == 'vit_b':
        checkpoint = "/home/lxy/pycharm_project/model_New/SAM_model_path/sam_vit_b_01ec64.pth"
        save_best_path = '/home/lxy/pycharm_project/model_New/SAM_model_path/iteration2_vitb/ACDC/sam_best_decoder.pth'
    ...
    #         )
    if args.use_iter==1:
        save_best_path = '/home/lxy/pycharm_project/model_New/SAM_FineTune/iteration1_vith_singCls_easy2/ratio9/sam_best_decoder.pth'
        sam_model = save_weight.load_partial_weight(
                model=sam_model,
                load_path=save_best_path
            )
    ...
    dataset = BaseDataSets_SAM(base_dir=args.root_path, transform=transforms.Compose([
                                RandomGenerator_SAM([256, 256])]
                                ),split="train",  fold='fold1', sup_type='scribble')
    dataloader = DataLoader(dataset, 1 , shuffle= False)

    experiment_name = args.exp
    result_path = f"result_all/{experiment_name}" + f"/{model_type}/{iter}/{args.num}/vis"
    sam_conf_LV = f"result_all/{experiment_name}" + f"/{model_type}/{iter}/{args.num}/Otherpl/LV"
    sam_conf_RV = f"result_all/{experiment_name}" + f"/{model_type}/{iter}/{args.num}/Otherpl/RV"
    sam_conf_MYO = f"result_all/{experiment_name}" + f"/{model_type}/{iter}/{args.num}/Otherpl/MYO"
    Vit_path = f"result_all/{experiment_name}" + f"/{model_type}/{iter}/{args.num}/Otherpl/vit_H"
    save_path = f"result_all/{os.path.split(experiment_name)[0]}"
    ...
    for step, sample_list in tqdm(enumerate(dataloader)):
        ...
        sampled_point_batch_background = contour_sample(scribble, 4, 20)
        ...
        # plt.imshow((label==1)[0].cpu().detach().numpy(),alpha=0.5,cmap="gray")
        ...
        # plt.axis('off')
        # plt.savefig(f'data/test/sample_point/{id[0]}_LV_.png', bbox_inches='tight', pad_inches=0)
        # plt.close()


        # plt.figure(figsize=(10,10))
        # plt.imshow(image,cmap='gray')
        # plt.imshow((label==2)[0].cpu().detach().numpy(),alpha=0.5,cmap="gray")
        ...
        # plt.axis('off')
        # plt.savefig(f'data/test/sample_point/{id[0]}_MYO_.png', bbox_inches='tight', pad_inches=0)
        # plt.close()
        ...
        # plt.figure(figsize=(10,10))
        # plt.imshow(image,cmap='gray')
        # plt.imshow((label==3)[0].cpu().detach().numpy(),alpha=0.5,cmap="gray")
        ...
        # plt.axis('off')
        # plt.savefig(f'data/test/sample_point/{id[0]}_RV_.png', bbox_inches='tight', pad_inches=0)
        # plt.close()
        ...
        lows_LV = lows_LV[ ... , : masks_LV.shape[1], : masks_LV.shape[2]]
        ...
        masks_RV, iou_predictions, lows_RV =  predictor.predict(
            ...
            multimask_output= False)
        lows_RV = lows_RV[ ... , : masks_RV.shape[1], : masks_RV.shape[2]]
        ...
        masks_MYO, iou_predictions, lows_MYO =  predictor.predict(
            ...
            multimask_output= False)
        lows_MYO = lows_MYO[ ... , : masks_MYO.shape[1], : masks_MYO.shape[2]]
        ...
        im.convert('L').save(f'{Vit_path}/{id[0][:-3]}.png')
        ...
        # plt.figure(figsize=(10,10))
        # plt.imshow(image1[0][0].cpu().numpy(),cmap='gray')
        # plt.imshow(label[0].cpu().numpy(), cmap=cmap1, alpha=0.5)
                
        # plt.axis('off')
        # plt.savefig(f'{result_path}/{id[0]}_GT.png', bbox_inches='tight', pad_inches=0)
        # plt.close()
        ...
        # plt.figure(figsize=(10,10))
        # plt.imshow(image1[0][0].cpu().numpy(),cmap='gray')
        # plt.imshow(masks_RV[0], cmap=cmap_RV, alpha=0.5)
        ...
        # # plt.imshow(scribble[0].cpu().detach().numpy(),alpha= 0.5,cmap=cmap)      
        # plt.axis('off')
        # plt.savefig(f'data/pred/{id[0]}.png', bbox_inches='tight', pad_inches=0)
        # plt.close()
        dices, HD95,IOU = calculate_metrics(label[0].cpu(), torch.from_numpy(mask), num_class)
        # print(dices)
    ...
    df = pd.DataFrame({  
    "name":f"SAM-SP:{args.model_type}_{iter}_{num}", 
    "Total_Dice": [avg_dice.astype(float).round(3)],
    "Total_HD95": [avg_HD95.astype(float).round(3)],
    "Total_IOU" : [avg_IOU.astype(float).round(3)],
    "category_0_Dice": [avg_dice_scores[0].cpu().detach().numpy().astype(float).round(3)],
    "category_0_HD95": [avg_HD95_scores[0].astype(float).round(3)],
    "category_0_IOU" : [avg_IOU_scores[0].cpu().detach().numpy().astype(float).round(3)],
    "category_1_Dice": [avg_dice_scores[1].cpu().detach().numpy().astype(float).round(3)],
    "category_1_HD95": [avg_HD95_scores[1].astype(float).round(3)],
    "category_1_IOU" : [avg_IOU_scores[1].cpu().detach().numpy().astype(float).round(3)],
    "category_2_Dice": [avg_dice_scores[2].cpu().detach().numpy().astype(float).round(3)],
    "category_2_HD95": [avg_HD95_scores[2].astype(float).round(3)],
    "category_2_IOU" : [avg_IOU_scores[2].cpu().detach().numpy().astype(float).round(3)]
})  
    # df = df.astype(float).round(3)
    empty_df.to_csv(f"{save_path}/val_contour_results.csv",mode = 'a', header= False, index= False)
    df.to_csv(f"{save_path}/val_contour_results.csv",mode = 'a', header= False, index= False)
